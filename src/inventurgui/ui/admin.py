        ...
        with ui.tab_panels(tabs, value=theme_tab).classes("w-full h-full p-0 fixed"):
            with ui.tab_panel(theme_tab):
                with ui.card().classes(
                    "md:w-1/2 w-full h-dvh bg-secondary text-base anitaliased text-primary"
                ) as background:
                    text = ui.markdown(lorem_ipsum).classes("color-primary text-justify")
                    with ui.row():
                        ui.color_input(
                            label="Foreground Color", on_change=lambda e: theme.set_primary_color(e.value)
                        ...
                        ).bind_value(background, "bg-color").classes("w-full md:w-auto xl:w-1/3 text-2xl font-bold")
                        ui.space()
