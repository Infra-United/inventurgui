...
from dataviewer.ui.helper.validators import IMG_URL_REGEX
...
async def cache_image(
    url_dict: dict[str, str], subfolder: str, filename: str, thumbnail_size: int = 600, compress: bool = False
):
...
def generate_qrcode(data: str, subfolder: str, filename: str):
    ...
    qr.add_data(data)
    img = qr.make_image(
        image_factory=StyledPilImage, module_drawer=RoundedModuleDrawer(), embedded_image_path=get_path(settings.logo)
    )
