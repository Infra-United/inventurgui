...
from dataviewer.ui.helper.reusable_elements import badge


def create_layout(warehouses: list[Selection]) -> LeftDrawer:
    # Drawers
    with ui.left_drawer(bordered=True).classes("gap-y-2 p-0 items-stretch").props("width=250") as ld:
        ui.space().classes("sm:hidden")
    with ld:
        drawer_menu(warehouses, settings.selection)
...
def main_menu(
    ...
    if settings.help.get("display"):
        help_btn: Button = ui.button(settings.help["label"], icon=settings.help["icon"])
